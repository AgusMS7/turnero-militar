...
import HeaderEstadisticasMedico from "./HeaderEstadisticasMedico";
...
/**
 * Componente principal para el panel de estadísticas.
 */
export default function PanelEstadisticasMedico() {
  ...
  const { data: doctor } = useGetOnePractitionerQuery({ id, token });
  ...
  return (
    <div className="w-full bg-gray-50">
      <HeaderEstadisticasMedico
        nombre={doctor?.name ?? "Medico"}
        foto={doctor?.urlImg ?? "/logo.png"}
      />
      <div className="w-full bg-gray-50">
        <div className="flex flex-col items-center py-10 gap-10">
          <div className="flex md:flex-row flex-col justify-between w-1/2">
            <h2 className="text-2xl font-bold mb-4">Estadísticas de {doctor?.name}</h2>
