"use client";
import { SocialWork, TokenWithEntity } from "@/app/definitions/definitions";
import { useCreateSocialWorkMutation } from "@/app/redux/api/socialWork.api";
...
export default function FormNuevaObraSocial() {
  ...
  const [createSocialWork, { isError: errorAlCrear, isSuccess: creada }] =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validarCampos()) {
      ...
      await createSocialWork(entityToken);
    }
  };
  ...
  if (creada) {
    Swal.fire({
      ...
      text: `La obra social ${obraSocial.name} fue agregada exitosamente`,
  ...
  if (errorAlCrear) {
    Swal.fire({
      ...
      text: "Error al agregar la obra social. Intente nuevamente más tarde",
